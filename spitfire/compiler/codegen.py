...
class CodeGenerator(object):
    ...
    def codegenASTTemplateNode(self, node):
        ...
        if self.options and self.options.cheetah_cheats:
            module_code.append_line(
                'from Cheetah.NameMapper import valueFromSearchList as resolve_placeholder')
            module_code.append_line(
            ...
            module_code.append_line(
                'from spitfire.runtime.udn import resolve_placeholder_with_locals')
            module_code.append_line(
        ...
        if not node.library:
            ...
            if not extends:
                ...
                if (self.options.base_template_full_import_path !=
                    ...
                    # below and is required regardless of which base template is being
                    # used.
        ...
        if not node.library and self.options and self.options.enable_psyco:
            module_code.append_line(
                'spitfire.runtime.template.enable_psyco(%(classname)s)' %
                vars())
    ...
    def codegenASTFromNode(self, node):
        ...
        if node.alias:
            ...
            return [CodeNode('from %(from_clause)s import %(import_clause)s' %
                             vars())]
        ...
        if self.options and self.options.cheetah_compatibility:
            ...
            if child_nodes and len(child_nodes) == 1 and not self.baked_mode:
                ...
                if isinstance(if_node,
                              IfNode) and not if_node.else_.child_nodes:
                    child_nodes = if_node.child_nodes
                    # Insert code that does:
                    #   if not ($test_expression):
                    #     return ''
                    new_if_condition = IfNode(UnaryOpNode(
