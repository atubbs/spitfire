...
class BaseTest(unittest.TestCase):
    ...
    def setUp(self):
        self.compiler = sptcompiler.Compiler(analyzer_options=self.options,
                                             xspt_mode=False,
...
class TestAssignAfterFilterWarning(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.compiler = sptcompiler.Compiler(analyzer_options=options,
                                             xspt_mode=False,
                                             compiler_stack_traces=True)
    ...
    """
        ...
        self.assertRaises(sptcompiler.Warning, optimization_analyzer.visit_ast,
                          ast_root)
...
class TestPartialLocalIdentifiers(BaseTest):
    ...
    """
        ...
        if_node_2.else_.append(AssignNode(
            IdentifierNode('foo'), LiteralNode(3)))
    ...
    """
        ...
        if_node_2.else_.append(AssignNode(
            IdentifierNode('foo'), LiteralNode(2)))
    ...
    """
        ...
        if_node_2.append(AssignNode(IdentifierNode('foo'), LiteralNode(1)))
        if_node_2.else_.append(AssignNode(
            IdentifierNode('bar'), LiteralNode(2)))
        if_node.append(if_node_2)
        if_node.else_.append(AssignNode(IdentifierNode('foo'), LiteralNode(3)))
        function_node.append(PlaceholderNode('foo'))
    ...
    """
        ...
        if_node_2.else_.append(AssignNode(
            IdentifierNode('baz'), LiteralNode(3)))
        if_node.else_.append(if_node_2)
...
class TestFinalPassHoistConditional(BaseTest):
    ...
    """
        ...
        def scope_setter(scope):
            ...
            scope.aliased_expression_map[FilterNode(IdentifierNode(
                '_rph_foo'))] = (IdentifierNode('_fph123'))
            scope.alias_name_set.add('_fph123')
            scope.alias_name_set.add('_rph_foo')

        def build_conditional_body(node):
            node.append(AssignNode(
                IdentifierNode('_rph_foo'), PlaceholderNode('foo')))
            node.append(AssignNode(
                IdentifierNode('_fph123'), FilterNode(IdentifierNode(
                    '_rph_foo'))))
            node.append(BufferWrite(IdentifierNode('_fph123')))
...
class TestHoistPlaceholders(BaseTest):
    ...
    """
        ...
        ast_root, function_node = self._build_function_template()
        ast_root.global_placeholders.add('foo')
        function_node.append(AssignNode(
            IdentifierNode('bar'), BinOpNode('+', PlaceholderNode('foo'),
                                             PlaceholderNode('foo'))))
    ...
    """
        ...
        condition = BinOpNode('or', PlaceholderNode('foo'),
                              PlaceholderNode('bar'))
...
class TestAssignSlice(BaseTest):
    ...
    """
        ...
        assign_node = AssignNode(
            SliceNode(
                IdentifierNode('foo'), LiteralNode(1)), LiteralNode(1))
...
#def foo
  $my_macro()
    ...
    """
        ...
        def pred(node):
            return bool(type(node) == IdentifierNode and node.name ==
...
#def foo($bar)
  #for $i in []
    ...
    """
        ...
        def pred(node):
            return type(node) == AssignNode and type(
