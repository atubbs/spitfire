...
class XHTML2AST(object):
                ...
                if attr_text:
                    if attr_ast:
                        node_list.append(TextNode(
                            u'<%(node_name)s %(attr_text)s' % vars()))
    ...
    def make_attr_node(self, attr):
        ...
        attr_ast = spitfire.compiler.util.parse(attr.nodeValue,
                                                'rhs_expression')
    ...
    def handle_define(self, dom_node, attr_name):
        ...
        ast = spitfire.compiler.util.parse(
    ...
    def handle_repeat(self, dom_node, attr_name):
        ...
        expr_ast = spitfire.compiler.util.parse(' '.join(expr_pieces[1:]),
                                                'rhs_expression')
        node_list = []
        ...
        fn = ForNode(
            ...
            ExpressionListNode([CallFunctionNode(
                IdentifierNode('enumerate'), ArgListNode([expr_ast]))]))
