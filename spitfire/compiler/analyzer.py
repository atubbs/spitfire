...
_ALLOWED_LIBRARY_NODES = (TextNode, ImplementsNode, ImportNode, WhitespaceNode,
                          LooseResolutionNode, AllowUndeclaredGlobalsNode,
                          CommentNode, DefNode, GlobalNode)
...
class SemanticAnalyzer(object):
    ...
    def analyzeTextNode(self, pnode):
        if pnode.child_nodes:
            self.compiler.error(
                SemanticAnalyzerError("TextNode can't have children"),
                pos=pnode.pos)
    ...
    def analyzeDefNode(self, pnode, allow_nesting=False):
        if (self.options.fail_nested_defs and not allow_nesting and
                not isinstance(pnode.parent, TemplateNode)):
            self.compiler.error(
    ...
    def analyzeBlockNode(self, pnode):
        ...
        function_node.expression = self.build_ast(PlaceholderNode(pnode.name))[
            0]
        ...
        if (self.compiler.enable_filters and
            ...
            if 'raw' in arg_map:
                ...
                if skip_filter:
                    ...
                    ph_expression = FilterNode(ph_expression,
                                               None,
                                               pos=pnode.pos)
                else:
                    ph_expression = FilterNode(
                        ph_expression,
                        arg_node_map.get('filter', DefaultFilterFunction),
                        pos=pnode.pos)
    ...
    def optimize_parsed_nodes(self, node_list):
        ...
        for n in node_list:
            ...
            # collapse adjacent TextNodes so we are calling these buffer writes
                  ...
                  isinstance(n, TextNode) and len(optimized_nodes) and
