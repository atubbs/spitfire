    ...
    def process_file(self, filename):
        ...
        if not self.options.quiet:
            ...
            if 'analyzed_tree' in self.options.debug_flags:
                print "analyzed_tree:"
                spitfire.compiler.visitor.print_tree(
                    self.compiler._analyzed_tree)
            if 'optimized_tree' in self.options.debug_flags:
                print "optimized_tree:"
                spitfire.compiler.visitor.print_tree(
                    self.compiler._optimized_tree)
            if 'hoisted_tree' in self.options.debug_flags:
                print "hoisted_tree:"
                spitfire.compiler.visitor.print_tree(
                    self.compiler._hoisted_tree)
        ...
        if self.options.test:
            ...
            try:
                if not self.options.quiet or self.options.compile:
                    ...
                    template_module = spitfire.runtime.import_module_symbol(
                        module_name)
            except Exception, e:
            ...
            test_output_path = os.path.join(
                os.path.dirname(filename), self.options.test_output,
                classname + '.txt')
...
if __name__ == '__main__':
    ...
    op.add_option('--test-output',
                  default='output',
                  help="directory for output")
    op.add_option('--accept-test-result',
                  action='store_true',
                  default=False,
    ...
    spitfire.runtime.udn.set_accelerator(options.enable_c_accelerator,
